...
void fmm<n, ord>::update_impl(int lin, int const * nb, int parent, double & U) {
  ...
  if (n == 2) {
    ...
    double U1 = fmin(
      nb[0] != -1 ? this->_U[nb[0]] : inf<double>,
      nb[2] != -1 ? this->_U[nb[2]] : inf<double>);

    double U2 = fmin(
      nb[1] != -1 ? this->_U[nb[1]] : inf<double>,
      nb[3] != -1 ? this->_U[nb[3]] : inf<double>);
    ...
    double sh = this->get_h()*this->_s[lin], sh_sq = sh*sh;
