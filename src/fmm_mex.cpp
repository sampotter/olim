...
void fmm_mex(double * out, bool * in, size_t M, size_t N, double h,
  ...
  } else if (type == marcher_type::olim_8pt_mp0) {
    m = std::make_unique<olim_8pt_mp0>(M, N, h, S);
  } else if (type == marcher_type::olim_8pt_mp1) {
    m = std::make_unique<olim_8pt_mp1>(M, N, h, S);
  } else if (type == marcher_type::olim_8pt_rhr) {
