...
template <>
fmm<2, ord> & olim_wrapper::olim<fmm<2, ord>>() {
  return _olim._fmm2;
}

template <>
fmm<3, ord> & olim_wrapper::olim<fmm<3, ord>>() {
  return _olim._fmm3;
}
...
status_e olim_wrapper_init(olim_wrapper ** w_ptr, olim_wrapper_params_s * p)
{
  ...
  if (p->nb == OLIM4) {
    ...
    if (p->F == RHR) {
      construct_olim<fmm<2, ord>>(w, p);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (p->nb == FMM3) {
    if (p->F == RHR) {
      construct_olim<fmm<3, ord>>(w, p);
    } else {
...
status_e olim_wrapper_deinit(olim_wrapper ** w_ptr)
{
  ...
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      destruct_olim<fmm<2, ord>>(w);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      destruct_olim<fmm<3, ord>>(w);
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_solve(olim_wrapper * w)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      w->olim<fmm<2, ord>>().solve();
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      w->olim<fmm<3, ord>>().solve();
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_step(olim_wrapper * w, int *lin)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      *lin = w->olim<fmm<2, ord>>().step();
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      *lin = w->olim<fmm<3, ord>>().step();
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_adjust(olim_wrapper * w, int * inds, double U)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      w->olim<fmm<2, ord>>().adjust(inds, U);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      w->olim<fmm<3, ord>>().adjust(inds, U);
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_add_src(olim_wrapper * w, int * inds, double U)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      w->olim<fmm<2, ord>>().add_src(inds, U);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      w->olim<fmm<3, ord>>().add_src(inds, U);
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_add_bd(olim_wrapper * w, int * inds)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      w->olim<fmm<2, ord>>().add_bd(inds);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      w->olim<fmm<3, ord>>().add_bd(inds);
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_add_free(olim_wrapper * w, int * inds)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      w->olim<fmm<2, ord>>().add_free(inds);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      w->olim<fmm<3, ord>>().add_free(inds);
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
                           ...
                           bool * empty)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      *empty = w->olim<fmm<2, ord>>().peek(value, lin);
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      *empty = w->olim<fmm<3, ord>>().peek(value, lin);
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_get_U_ptr(olim_wrapper * w, double ** U_ptr)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      *U_ptr = w->olim<fmm<2, ord>>().get_U_ptr();
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      *U_ptr = w->olim<fmm<3, ord>>().get_U_ptr();
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_get_s_ptr(olim_wrapper * w, double ** s_ptr)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      *s_ptr = w->olim<fmm<2, ord>>().get_s_ptr();
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      *s_ptr = w->olim<fmm<3, ord>>().get_s_ptr();
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
...
status_e olim_wrapper_get_state_ptr(olim_wrapper * w, char ** state_ptr)
{
  if (w->nb == OLIM4) {
    ...
    if (w->F == RHR) {
      *state_ptr = w->olim<fmm<2, ord>>().get_state_ptr();
    } else {
      throw std::runtime_error("FMM2 requires quad == RHR");
    }
  } else if (w->nb == FMM3) {
    if (w->F == RHR) {
      *state_ptr = w->olim<fmm<3, ord>>().get_state_ptr();
    } else {
      throw std::runtime_error("FMM3 requires quad == RHR");
    }
  }
