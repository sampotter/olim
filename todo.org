...
* General refactoring
  1. [ ] Add T = template parameter for double/float
  2. [ ] Remove second argument from `sqp_bary': i.e., use only one
  ...
  3. [ ] Don't actually need to store qr_wkspc in cost_functor...
  4. [ ] Reduce line count in tests using generic lambdas...
  5. [ ] Remove STL headers
  6. [ ] Remove macros
     - [ ] P*** macros -> b'***' literals
  7. [ ] Global p_fac -> pf
  8. [ ] Global s_fac -> sf
