...
* General refactoring
  1. [ ] Remove all instances of EIKONAL and replace with OLIM
  2. [ ] Collapse marcher and marcher_3d into one class
  3. [ ] Some renaming
     - [ ] olim -> olim2
     - [ ] olim3d -> olim3
     - [ ] basic_marcher -> fmm2
  ...
  4. [ ] T -> U everwhere
  5. [ ] Add T = template parameter for double/float
  6. [ ] Distinguish between indices
  ...
  7. [ ] Remove second argument from `sqp_bary': i.e., use only one
  ...
  8. [ ] Don't actually need to store qr_wkspc in cost_functor...
  9. [ ] Reduce line count in tests using generic lambdas...
