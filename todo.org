...
* General refactoring
  ...
  4. [ ] speed -> slowness
  5. [ ] T -> U everwhere
  6. [ ] Add T = template parameter for double/float
  7. [ ] Distinguish between indices
  ...
  8. [ ] Remove second argument from `sqp_bary': i.e., use only one
  ...
  9. [ ] Don't actually need to store qr_wkspc in cost_functor...
