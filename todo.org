...
* General refactoring
  ...
  3. [ ] Rewrite as much as possible in terms of vector class
     - [ ] Add a `range' iterator to simplify loops and make them
       coordinate-free
  4. [ ] Simplify updates.tri.hpp... It's way more complicated than it
     needs to be... 330 lines where it should be around 100-150
  5. [ ] Collapse marcher and marcher_3d into one class
  6. [ ] Some renaming
     - [ ] olim -> olim2
     - [ ] olim3d -> olim3
     - [ ] basic_marcher -> fmm2
  ...
  8. [ ] get_value -> get_U
  9. [X] T -> U everwhere
  10. [ ] Add T = template parameter for double/float
  11. [ ] Distinguish between indices
  ...
  12. [ ] Remove second argument from `sqp_bary': i.e., use only one
  ...
  13. [ ] Don't actually need to store qr_wkspc in cost_functor...
  14. [ ] Reduce line count in tests using generic lambdas...
  15. [ ] Remove STL headers
  16. [ ] Remove macros
      - [ ] P*** macros -> b'***' literals
  17. [ ] Global p_fac -> pf
  18. [ ] Global s_fac -> sf
