* Smart mesh
  ...
  - Things to consider longer term:
    ...
    - [ ] Actual predictive meshing (i.e., is there a way to avoid
      two passes?)
* General
  - [ ] Figure out source of error in marchers
    - [ ] Make radial error plots, compare with Masha's
...
* Implementation
  ...
  - [ ] Make sure all implementations of update_node_value_impl are
    short-circuiting and continuing for performance
  - [ ] Figure out how to conditionally avoid using the _S_cache by
    conditionally setting function pointer
  - [ ] Remove adjust_heap_entry and insert_into_heap
  - [ ] See which protected member functions in fast_marcher(_3d) can
    be made private
  - [ ] Replace use of std::vector<double> for _S_cache with a double *
  - [ ] Read Section 3 of "imm841.pdf" which talks about how to
  ...
  - [ ] Use multiple inheritance to simplify class hierarchy
